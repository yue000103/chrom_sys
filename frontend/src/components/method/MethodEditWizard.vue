                        ...
                        <el-row :gutter="20">
                            <el-col :span="12">
                                <el-form-item label="起始比例 (%)">
                                    <el-input-number
                                        v-model="methodData.autoGradient.startRatio"
                                        :min="0"
                                        :max="100"
                                        style="width: 100%"
                                    />
                            ...
                            <el-col :span="12">
                                <el-form-item label="结束比例 (%)">
                                    <el-input-number
                                        v-model="methodData.autoGradient.endRatio"
                                        :min="0"
                                        :max="100"
                                        style="width: 100%"
                                    />
                        ...
                        <el-row :gutter="20">
                            <el-col :span="12">
                                <el-form-item label="N1柱体积数">
                                    <el-input-number
                                        v-model="methodData.autoGradient.n1Volumes"
                                        :min="0.1"
                                        :max="20"
                                        :step="0.1"
                                        :precision="1"
                            ...
                            <el-col :span="12">
                                <el-form-item label="梯度速率">
                                    <el-input-number
                                        v-model="methodData.autoGradient.gradientRate"
                                        :min="0.1"
                                        :max="10"
                                        :step="0.1"
                                        :precision="1"
                        ...
                        <el-row :gutter="20">
                            <el-col :span="12">
                                <el-form-item label="峰阈值">
                                    <el-input-number
                                        v-model="methodData.autoGradient.peakThreshold"
                                        :min="0.001"
                                        :max="1"
                                        :step="0.001"
                                        :precision="3"
                            ...
                            <el-col :span="12">
                                <el-form-item label="柱体积 (mL)">
                                    <el-input-number
                                        v-model="methodData.autoGradient.columnVolume"
                                        :min="1"
                                        :max="200"
                                        style="width: 100%"
                                    />
                        ...
                        <el-row :gutter="20">
                            <el-col :span="12">
                                <el-form-item label="SG滤波窗口">
                                    <el-input-number
                                        v-model="methodData.autoGradient.sgWindow"
                                        :min="3"
                                        :max="21"
                                        :step="2"
                                        style="width: 100%"
                            ...
                            <el-col :span="12">
                                <el-form-item label="SG滤波阶数">
                                    <el-input-number
                                        v-model="methodData.autoGradient.sgOrder"
                                        :min="1"
                                        :max="5"
                                        style="width: 100%"
                                    />
                        ...
                        <el-row :gutter="20">
                            <el-col :span="12">
                                <el-form-item label="基线窗口">
                                    <el-input-number
                                        v-model="methodData.autoGradient.baselineWindow"
                                        :min="5"
                                        :max="50"
                                        style="width: 100%"
                                    />
                            ...
                            <el-col :span="12">
                                <el-form-item label="K因子">
                                    <el-input-number
                                        v-model="methodData.autoGradient.kFactor"
                                        :min="0.1"
                                        :max="10"
                                        :step="0.1"
                                        :precision="1"
                        ...
                        <el-form-item>
                            ...
                            >
                                ...
                                >
                                    <template #default="scope">
                                        <el-input-number
                                            ...
                                            size="small"
                                            style="width: 100%"
                                            @change="validateGradientRow(scope.$index)"
                                        />
                                    </template>
                                </el-table-column>
                                <el-table-column
                                ...
                                <el-table-column label="总和" width="70">
                                    <template #default="scope">
                                        <el-text
                                            :type="getGradientSumType(scope.row)"
                                ...
                                <el-table-column label="操作" width="80">
                                    <template #default="scope">
                                        <el-button
                                            size="small"
                                            type="danger"
                                            @click="removeGradientStep(scope.$index)"
                                            >删除</el-button
                                        >
                                    </template>
                                </el-table-column>
...
export default {
    ...
    setup(props, { emit }) {
        ...
        // 将后端方法数据转换为前端表单数据格式
        const convertMethodToFormData = (method) => {
            if (!method) return getDefaultMethodData();

            const originalData = method.originalData || method;

            // 解析检测器波长
            let wavelengthA = 254, wavelengthB = 280;
            if (originalData.detector_wavelength) {
                const wavelengthStr = Array.isArray(originalData.detector_wavelength)
            ...
            if (originalData.auto_gradient_params) {
                try {
                    const parsedParams = JSON.parse(originalData.auto_gradient_params);
                    autoGradient = { ...autoGradient, ...parsedParams };
                } catch (e) {
                    console.warn('解析自动梯度参数失败:', e);
                }
            }
            ...
            let manualGradient = [
                {
                    ...
                    solutionC: 0,
                    solutionD: 0,
                    flowRate: originalData.flow_rate_ml_min || 1.0,
                },
                {
                    time: originalData.run_time_min || 30,
                    solutionA: 20,
                    solutionB: 80,
                    solutionC: 0,
                    solutionD: 0,
            ...
            if (originalData.gradient_time_table) {
                try {
                    // 解析时间表格式：如 "0:80:20:0:0,30:20:80:0:0"
                    const steps = originalData.gradient_time_table.split(',');
                    if (steps.length > 0) {
                        manualGradient = steps.map(step => {
                            const parts = step.split(':');
                            return {
                                time: parseInt(parts[0]) || 0,
                                solutionA: parseInt(parts[1]) || 0,
                                solutionB: parseInt(parts[2]) || 0,
                                solutionC: parseInt(parts[3]) || 0,
                                solutionD: parseInt(parts[4]) || 0,
                                flowRate: originalData.flow_rate_ml_min || 1.0,
                            };
                        });
                    }
                } catch (e) {
                    console.warn('解析手动梯度时间表失败:', e);
                }
            }

            return {
                name: originalData.method_name || '未命名方法',
                smlies: "", // 可以根据需要设置
                type: "user",
                column_id: originalData.column_id,
                flowRate: originalData.flow_rate_ml_min || 1.0,
                runTime: originalData.run_time_min || 30,
                wavelengthA: wavelengthA,
                wavelengthB: wavelengthB,
                gradientMode: originalData.gradient_elution_mode || "auto",
        ...
        const methodData = ref(convertMethodToFormData(props.method));

        // 监听props.method的变化，更新methodData
        watch(
            () => props.method,
            (newMethod) => {
                if (newMethod) {
                    methodData.value = convertMethodToFormData(newMethod);
                }
            },
        ...
        const addGradientStep = () => {
            ...
            methodData.value.manualGradient.push({
                ...
                solutionC: 0,
                solutionD: 0,
                flowRate: methodData.value.flowRate,
            });
        };
        ...
        const validateAllGradientRows = () => {
            if (methodData.value.gradientMode !== "manual") return { isValid: true, errors: [] };
        ...
        // 更新方法
        const updateMethod = () => {
            // 构造更新请求数据，与创建方法使用相同的数据格式
            const updateData = {
                method_name: methodData.value.name,
                column_id: methodData.value.column_id,
                flow_rate_ml_min: methodData.value.flowRate,
                run_time_min: methodData.value.runTime,
                detector_wavelength: `${methodData.value.wavelengthA}nm`,
                peak_driven: false, // 根据需要设置
                gradient_elution_mode: methodData.value.gradientMode,
                gradient_time_table: methodData.value.gradientMode === 'manual'
                    ? methodData.value.manualGradient.map(row =>
                        `${row.time}:${row.solutionA}:${row.solutionB}:${row.solutionC}:${row.solutionD}`
                      ).join(',')
                    : '',
                auto_gradient_params: methodData.value.gradientMode === 'auto'
                    ? JSON.stringify(methodData.value.autoGradient)
                    : ''
            };
